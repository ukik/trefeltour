...
class TravelReservationsController extends Controller
{
    ...
    public function browse(Request $request)
    {
        try {
            ...
            $data = \TravelReservations::with('badasoUsers')->orderBy('id','desc')->paginate(request()->perPage);
    ...
    public function add(Request $request)
    {
        ...
        try {
            ...
            $data = [
                ...
                'starting_location' => $req['starting_location'] ,
                'arrival_location' => $req['arrival_location'] ,
                'is_reserved' => (!isAdmin() || !isSuperAdmin()) ?  'false' : ($req['is_reserved'] ? 'true' : 'false'),
                'is_cancel' => (!isAdmin() || !isSuperAdmin()) ?  'false' : ($req['is_cancel'] ? 'true' : 'false'),
                'code_table' => ($slug) ,
                'uuid' => ShortUuid(),
            ];
